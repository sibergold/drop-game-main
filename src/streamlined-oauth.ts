...
import { CENTRAL_OAUTH_CONFIG, isCentralConfigValid, buildCentralOAuthUrl, fetchKickUserInfo, exchangeCodeForToken, testNetlifyFunction } from "./central-config";
...
class StreamlinedKickOAuth {
	...
	private init() {
		...
		testBtn.textContent = '🔧 Test Netlify Functions';
		testBtn.className = 'big-button';
		testBtn.style.marginTop = '10px';
		testBtn.style.backgroundColor = '#ff6b35';
		testBtn.addEventListener('click', async () => {
			testBtn.disabled = true;
			testBtn.textContent = '🔄 Testing...';

			const isWorking = await testNetlifyFunction();

			testBtn.disabled = false;
			testBtn.textContent = isWorking ? '✅ Functions Working' : '❌ Functions Failed';
		...
		// Add test button to setup section
		this.setupSection.appendChild(testBtn);
	...
	private async handleOAuthReturn() {
		try {
			...
			if (hs.error) {
				throw new Error(`OAuth error: ${hs.error} - ${hs.error_description || 'No description'}`);
			}
